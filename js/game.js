...
var ballRadius = 53; //ball radius
var totalBall = 100; //total balls//Rodolfo: Se agrega la variable totalBall para que el juego sea dinamico
var rotateBall = true; //rotate balls in 3d angle
var numberStartZero = true; //number start from zero instead of one//Rodolfo: Se agrega la variable numberStartZero para que los numeros empiecen en 0 o en 1
...
var bonusBall = false; //toggle bonus ball//Rodolfo: Se agrega la variable bonusBall para activar o desactivar la bola bonus
...
var exitMessage = 'Transmision Finalizada Felicidades a los Ganadores'; //go to main page message
var resultCompleteText = 'Felicidades a los Ganadores'; //result complete text display
var resultFailText = 'Gracias por Jugar Quinas';  //result fail text display

//Social share, [SCORE] will replace with game score
var shareEnable = false; //toggle share//Rodolfo: Se agrega la variable shareEnable para activar o desactivar el boton de compartir
...
var optimizeData = {balls:100};//Modificado por Rodolfo para optimizar el juego agragando un parametro para el numero de bolas
...
gameData.revealArray = [40]; //se coloca el numero ganador

	itemBarUser.visible = false;
	buttonSphereStart.visible = true;// modificar para que no se vea el boton de girar
	buttonLucky.visible = true;// modificar para que no se vea el boton de lucky///
	cardContainer.visible = false;// modificar para que no se vea el carton
...
function startSpin(){
	...
	if(gameData.selectArray.length == score_arr.length){
		...
		itemBarUser.visible = false;///no se ve el itembar
		buttonSphereStart.visible = true; ////////no se oculta el boton de girar
		buttonLucky.visible = true;/////////// no se oculta el boton de lucky
		cardContainer.visible = false;
		tableContainer.visible = false;// se oculta la informacion de la tarjeta
		
		selectTitleTxt.text = prizeTableDisplay;
		
		shuffle(gameData.numberArray);
		//setSelectBalls();// SE bloquean los numeros al asar que se van a mostrar
		gameData.spin = true;////
		...
		for(var n=0; n<gameData.revealArray.length; n++){
			...
			for(var p=0; p<gameData.ballNumber.length; p++){
				if(currentNumber == gameData.ballNumber[p].number){
					gameData.ballNumber[p].status = false/////////;
				}
			}
		}
...
function toggleOption(){
	if(optionsContainer.visible){
		...
		optionsContainer.visible = true;//se desactiva el boton del menu
	}
}
